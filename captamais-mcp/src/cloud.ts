...
export function prepareMeeting(config: CaptaMaisConfig, lead: Lead): Promise<MeteredResult<{ prep: string }>> {
  return callCloud<{ prep: string }>(config, '/api/mcp/ai/prepare-meeting', {
    lead: {
      name: lead.name,
      email: lead.email,
      phone: lead.phone,
...
export type GoogleStatus = { connected: boolean; gmail: boolean; calendar: boolean; email?: string };
export function googleStatus(config: CaptaMaisConfig): Promise<MeteredResult<GoogleStatus>> {
  return callCloud<GoogleStatus>(config, '/api/mcp/integrations/google/status', null, 'GET');
}

export type GoogleEvent = { eventId?: string; htmlLink?: string; meetLink?: string };
export function createGoogleCalendarEvent(
  config: CaptaMaisConfig,
  lead: Lead,
  activity: { localId: number; type: string; title: string | null; startAt: string; notes: string | null },
): Promise<MeteredResult<GoogleEvent>> {
  return callCloud<GoogleEvent>(config, '/api/mcp/integrations/google/calendar/events', {
    activity,
